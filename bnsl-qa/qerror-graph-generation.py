...
for i, (sa_f, sqa_f) in enumerate(zip(sa_files, sqa_files)):
    ...
    # Standardise perfect predictions to 1.0
    ...
    # Dots only: linestyle="None" removes the connecting line.
    ax.plot(
        x,
        sa_q,
        marker="o",
        linestyle="None",
        color=sa_blues[i % len(sa_blues)],
        label=f"SA - {sa_reads} Reads - Trials {trials}"
    )
    ...
    ax.plot(
        ...
        marker="s",
        linestyle="None",
        color=sqa_purples[i % len(sqa_purples)],
...
max_queries = max(max_queries, len(bn_q), len(pg_q))

# Dots only for benchmark estimators as well.
ax.plot(
    x_bench,
    bn_q,
    marker="^",

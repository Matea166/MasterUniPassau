...
class BayesianNetwork(nx.DiGraph):
    ...
    def __init__(self, incoming_graph_data=None, cl_max_rows=30000,
                 random_state=None, unique_ratio_limit=1.0):
    ...
    def fit(self, relation):
        ...
        if len(relation) > self.cl_max_rows:
            r = relation.sample(n=self.cl_max_rows,
    ...
    def update(self, relation, by_m=30, by_n=30, on_m=30, on_n=30):
        ...
        def walk(node, parent):
            ...
            self.nodes[node]['dist'] = (
                cpd.CPD(by_m, by_n, on_m, on_n).fit(by, on)
            )
        ...
        self.nodes[root]['dist'] = (
            histogram.Histogram(on_m, on_n)
            .fit(root_values.values.tolist())
        )
